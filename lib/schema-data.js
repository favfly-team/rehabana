 ...
 *
 * @param {object} doctor - A profile object from `data/doctors.js`
 * @returns {object}
 */
export const generatePhysicianSchema = (doctor) => {
  ...
  const alumni = (doctor.education ?? [])
    ...
    .filter((institution) => institution && !/placeholder/i.test(institution))
    .map((institution) => ({
  ...
  const memberOf = (doctor.memberships ?? [])
    .filter((membership) => membership && !/placeholder/i.test(membership))
  ...
  return {
    ...
    ...(doctor.specialities?.length > 0 && {
      knowsAbout: doctor.specialities.map((item) => item.label ?? item),
    }),
    worksFor: { "@id": "https://rehabana.com/#organization" },
    affiliation: { "@id": "https://rehabana.com/#organization" },
    ...(doctor.social?.linkedin && { sameAs: [doctor.social.linkedin] }),
  };
};
